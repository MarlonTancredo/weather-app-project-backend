...
export async function fetchWeather(location: LocationDetails) {
  ...
  const forecast: LocationForecast = {
    ...
    current_day: {
      ...
      wind_from_direction:
        data.properties.timeseries[1].data.instant.details.wind_from_direction, // degrees - (0° is north, 90° east, etc.)
      wind_speed: data.properties.timeseries[1].data.instant.details.wind_speed, // m/s
      symbol_code:
        data.properties.timeseries[1].data.next_12_hours.summary.symbol_code,
    },
    timeseries: [],
  };
